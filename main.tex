...
Additional category-level percentage statistics are reported in Appendix~\ref{app:chemgen_category_stats}.
...
\end{table*}
    ...
    \subsection{ChemGen Construction}
    ...
    \subsection{Category-Level Dataset Statistics}
    \label{app:chemgen_category_stats}
    We report two complementary percentage-based views of ChemGen constraints. Table~\ref{tab:chemgen_constraint_category_share} quantifies the constraint-level composition of the dataset, where percentages are computed with respect to the total number of constraints ($N=4{,}599$); this table displays how frequently each category appears among all requirement instances. 
    
    Table~\ref{tab:chemgen_sample_category_share} quantifies sample-level coverage, where percentages are computed with respect to the total number of samples ($N=779$); this table answers in how many dataset entries each category appears at least once. Because a single sample can include multiple categories, sample-level percentages are not expected to sum to 100\%. Together, these views distinguish category prevalence at the requirement-instance level from category co-occurrence breadth across samples.
    ...
    \begin{table*}[h]
        ...
        \begin{tabular}{lrr}
            \toprule
            \textbf{Attribute category} & \textbf{Count} & \textbf{Share of all constraints (\%)} \\
            \midrule
            Functional group constraints (contain/not contain) & 1020 & 22.18 \\
            Covalently bonded unit count & 543 & 11.81 \\
            Topological polar surface area & 538 & 11.70 \\
            Molecular weight & 531 & 11.55 \\
            Heavy atom count & 530 & 11.52 \\
            Rotatable bond count & 516 & 11.22 \\
            Rings: exact count with specified sizes & 462 & 10.05 \\
            Rings: bounded count per ring size & 459 & 9.98 \\
            \midrule
            \textbf{Total} & \textbf{4599} & \textbf{100.00} \\
            \bottomrule
        \end{tabular}
        \caption{Share of all ChemGen constraints by category.}
        \label{tab:chemgen_constraint_category_share}
    \end{table*}
    ...
    \begin{table*}[h]
        ...
        \begin{tabular}{lrr}
            \toprule
            \textbf{Attribute category} & \textbf{Samples with category} & \textbf{Share of all samples (\%)} \\
            \midrule
            Functional group constraints (contain/not contain) & 677 & 86.91 \\
            Covalently bonded unit count & 543 & 69.70 \\
            Topological polar surface area & 538 & 69.06 \\
            Molecular weight & 531 & 68.16 \\
            Heavy atom count & 530 & 68.04 \\
            Rotatable bond count & 516 & 66.24 \\
            Rings: exact count with specified sizes & 462 & 59.31 \\
            Rings: bounded count per ring size & 459 & 58.92 \\
            \bottomrule
        \end{tabular}
        \caption{Share of ChemGen samples containing at least one constraint from each category (N=779).}
        \label{tab:chemgen_sample_category_share}
    \end{table*}
